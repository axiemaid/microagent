...
// Parse --agent-dir args or --scan-dir
const AGENTS = [];
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--agent-dir' && args[i + 1]) {
    ...
    AGENTS.push({ name: path.basename(dir), dir, color: COLORS[AGENTS.length % COLORS.length] });
  } else if (args[i] === '--scan-dir' && args[i + 1]) {
    scanDir = path.resolve(args[++i]);
  }
}

// Scan directory for agent dirs (dirs containing wallet.json)
if (scanDir && fs.existsSync(scanDir)) {
  for (const entry of fs.readdirSync(scanDir)) {
    const dir = path.join(scanDir, entry);
    if (fs.statSync(dir).isDirectory() && fs.existsSync(path.join(dir, 'wallet.json'))) {
      // Skip if already added via --agent-dir
      if (!AGENTS.some(a => a.dir === dir)) {
        AGENTS.push({ name: entry, dir, color: COLORS[AGENTS.length % COLORS.length] });
      }
    }
  }
}
...
if (!AGENTS.length) {
  console.error('Usage: node viewer.cjs --agent-dir <path> [--agent-dir <path2>] [--scan-dir <dir>]');
  console.error('  --agent-dir  Path to an agent data directory (repeatable)');
  console.error('  --scan-dir   Scan a directory for agent subdirs (containing wallet.json)');
  process.exit(1);
}

console.log(`Viewing ${AGENTS.length} agent(s): ${AGENTS.map(a => a.name).join(', ')}`);

function getAgentInfo(agent) {
  const wallet = JSON.parse(fs.readFileSync(path.join(agent.dir, 'wallet.json'), 'utf8'));
  const statePath = path.join(agent.dir, 'state.json');
  const state = fs.existsSync(statePath) ? JSON.parse(fs.readFileSync(statePath, 'utf8')) : { lastBalance: 0, loopCount: 0 };
...
.agents { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; margin-bottom: 24px; }
  ...
  if (req.url === '/api/conversation') {
    ...
    AGENTS.forEach((a) => {
      ...
      for (const e of entries) {
        conversation.push({ agent: a.name, color: a.color, time: e.time, text: e.text, txid: e.txid, fee: e.fee });
      }
    });
